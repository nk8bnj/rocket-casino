...
class MinesApi {
  ...
  private generateMinePositions(count: number): number[] {
  ...
  private generateServerSeedHash(): string {
    return `hash_${Math.random().toString(36).slice(2, 18)}`;
  }
  ...
  async startGame(
    ...
    const minePositions = this.generateMinePositions(request.mines_count);
    const serverSeed = `seed_${Math.random().toString(36).slice(2, 22)}`;
    ...
    return {
      game_id: gameId,
      server_seed_hash: this.generateServerSeedHash(),
  ...
  async revealTile(
    ...
    const game = this.activeGames.get(request.game_id);
  ...
  async cashOut(
    ...
    this.activeGames.delete(request.game_id);
